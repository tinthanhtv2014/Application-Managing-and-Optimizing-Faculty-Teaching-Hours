...
const addCoQuyDinh = async (req, res) => {
  const { MA_QUY_DOI, MA_LOAI_DANH_MUC, MA_LOAI_TAC_GIA, SO_TAC_GIA } =
    req.body;
  ("check data MA_QUY_DOI =>", MA_QUY_DOI);
  ("check data MA_LOAI_DANH_MUC =>", MA_LOAI_DANH_MUC);

  ("check data MA_LOAI_TAC_GIA =>", MA_LOAI_TAC_GIA);
  ("check data SO_TAC_GIA =>", SO_TAC_GIA);
  try {
