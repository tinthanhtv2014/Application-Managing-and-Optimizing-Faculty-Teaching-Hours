...
const selectLoaiDanhMuc_TEN_LOAI_DANH_MUC = async (TEN_LOAI_DANH_MUC) => {
    try {
        let [results, fields] = await pool.execute(`SELECT * FROM loai_danh_muc WHERE TEN_LOAI_DANH_MUC = ?`, [TEN_LOAI_DANH_MUC]);
        return results;
    } catch (error) {
        return {
            EM: "Lỗi services selectLoaiDanhMuc",
...
const deleteLoaiDanhMuc = async (id) => {
    try {
        let [results, fields] = await pool.execute(`DELETE FROM loai_danh_muc WHERE MA_LOAI_DANH_MUC = ?`, [id]);
        return {
